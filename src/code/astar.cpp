...
void Astar::runAstar(){
    ...
    if(hfunc == 4){  //dfs
        ...
        qDebug("path列表的长度：%lld", path.size());

        for (int i = 0; i < path.size(); i++) { //遍历path列表
            if (path[i].isEmpty()) { //判断路径是否为空
                continue; //跳过该路径
            }
            qDebug() << "路径" << i + 1 << ":"; //输出路径的序号
            QSet<QPair<int,int>> visited; //定义一个集合，用于记录已经输出过的节点
            for (int j = 0; j < path[i].size(); j++) { //遍历路径中的每个节点
                QPair<int,int> pair(path[i][j].x,path[i][j].y); //定义一个二元组，用于表示当前节点的坐标
                if (visited.contains(pair)) { //判断当前节点是否已经输出过
                    continue; //跳过该节点
                }
                qDebug() << "(" << path[i][j].x << "," << path[i][j].y << ")"; //输出节点的坐标
                visited.insert(pair); //将当前节点添加到访问集合中
            }
        }
    }
    ...
    if(hfunc==1 || hfunc==2 || hfunc==3){   //A*算法
        QElapsedTimer time;
        time.start();
        while(1){
            ...
            for(int i=1; i<=4; i++){    //四方向，i=1，2，3，4分别代表上右下左
                ...
                if(anode[optrx][optry].g==0){   //如果相邻节点未被访问过，说明初始g=0
                ...
                if(anode[optrx][optry].g!=0){ //factor=1，2分别表示考虑安全距离和考虑拐角
                    if(factor==2 || factor==1){
                        int penalty = 0;
                        int alpha = 0;
                        if((factor == 2 and (current.lastpoint.x() - current.x) * (current.x - optrx) + (current.lastpoint.y() - current.y) * (current.y - optry) != 0)){
                            penalty = penal;
                        }
                        //选择障碍数因子alpha
                        if (factor == 1){
                            ...
                            case 1: //静态alpha
                                ...
                                break;
                            case 2: //考虑相邻节点
                                int beta = current.blocks + anode[optrx][optry].blocks;
                                alpha = beta - 4;
                                break;
                            }
                        }
                        if(anode[optrx][optry].g>current.g+10-penalty+alpha and anode[optrx][optry].isInOpenList){
                            anode[optrx][optry].g=current.g+10;
                            anode[optrx][optry].cost=anode[optrx][optry].g+anode[optrx][optry].h-penalty+alpha;   //f=g+h+penalty
                            anode[optrx][optry].lastpoint.setX(current.x);
                            anode[optrx][optry].lastpoint.setY(current.y);
