...
void MainWindow::setfactor(){
    if(angleAction->isChecked()){
        bool ok;
        int num = QInputDialog::getInt(this, tr("提示"), tr("请输入拐角权值："), 1, 1, 10, 2, &ok);
        if (ok){
            map->setpenalty(num);
        }
        //考虑拐角问题
...
void MainWindow::setStatusBar(){
    ...
    hfuncStatus=new QLabel("启发函数：对角最短距离");
...
void MainWindow::setStatusBar(int x,int y,int dir,int hfunc){
    ...
    switch(hfunc){
    case 1:
        hfuncStatus->setText(QString("启发函数：对角最短距离|算法：A*"));
        break;
    case 2:
    ...
    case 5:
        ...
        break;
    case 6:
        hfuncStatus->setText(QString("|算法：迭代深搜"));
        break;
    }
}
